...
function calculateSubtotal() {
	let total = 0;
	for (let x of ITEMS){
		const quantity = parseInt(document.getElementById(x +"-quantity").value);
	    const price = parseFloat(document.getElementById(x +"-price").innerText);
...
function calculateSalesTax() {
	let total = 0;
	total = roundMoney(calculateSubtotal() * getSalesTaxRateForState(stateAbb))	
	return total; // TODO calculateSalesTax
}

function getSalesTaxRateForState(state) {
	for (let x in SALES_TAX) {
		if (state === x)
			return SALES_TAX[x];
	}
